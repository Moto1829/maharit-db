...
def save_report(all_results: list[BenchResult], args, output_path: str):
    ...
    write_results = [r for r in all_results if r.name.startswith("CREATE")]
    for r in write_results:
        lines.append(f"- **{r.name}**: {r.throughput:,.0f} ops/s (avg {r.latency_ms:.2f} ms)")
    ...
    read_results = [r for r in all_results if not r.name.startswith("CREATE")]
    for r in read_results:
        if r.count == 0:
            continue
        lines.append(f"- **{r.name}**: {r.throughput:,.0f} ops/s (avg {r.latency_ms:.2f} ms)")

    lines += ["", "---", f"*Generated by `scripts/benchmark.py`*", ""]
