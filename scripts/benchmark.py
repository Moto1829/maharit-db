...
使い方:
  ...
  python3 scripts/benchmark.py --no-ai          # Claude 分析をスキップ
"""
...
def _results_to_prompt(all_results: list[BenchResult], args) -> str:
    """Claude に渡す分析プロンプトを生成する"""
    rows = "\n".join(
        f"  - {r.name}: {r.count:,} rows, {r.elapsed:.3f}s, "
        f"{r.throughput:,.0f} ops/s, {r.latency_ms:.3f} ms/op"
        for r in all_results if r.count > 0
    )
    return f"""以下は MaharitDB (Rust 製グラフデータベース) の性能ベンチマーク結果です。
接続先: {args.host}:{args.port} / ノード数: {args.nodes:,}

計測結果:
{rows}

この結果を分析し、Markdown 形式で以下の内容を出力してください。
各セクションに ## 見出しをつけてください。

1. **パフォーマンス概評** — 全体的な傾向と特徴的な数値
2. **ボトルネック分析** — 遅い操作とその推定原因（アーキテクチャ的な観点で）
3. **改善提案** — 具体的な実装上の改善点（インデックス、クエリ最適化、並列化など）
4. **比較観点** — 一般的なグラフDB（Neo4j等）と比べた評価

出力は Markdown のみ（コードブロックで囲まない）で、日本語で記述してください。"""


def analyze_with_claude(all_results: list[BenchResult], args) -> str | None:
    """Claude API を呼び出してベンチ結果を分析する。失敗時は None を返す。"""
    try:
        import anthropic
    except ImportError:
        print(f"  {YELLOW}⚠{RESET}  anthropic パッケージ未インストール。分析をスキップします。")
        print(f"      pip install anthropic  でインストールできます。")
        return None

    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        print(f"  {YELLOW}⚠{RESET}  ANTHROPIC_API_KEY が未設定。分析をスキップします。")
        return None

    section("Claude による改善点分析")
    print(f"  分析中...", end="", flush=True)

    client = anthropic.Anthropic(api_key=api_key)
    prompt = _results_to_prompt(all_results, args)
...
def main():
    ...
    ai_analysis = None
    if not args.no_ai:
        ai_analysis = analyze_with_claude(all_results, args)

    # レポート保存
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = args.output or f"benchmark_reports/bench_{ts}.md"
    save_report(all_results, args, output_path, ai_analysis)
